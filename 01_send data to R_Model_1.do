...
replace PostHbA1cSDGroup = 2.22 if Analysis_bin == 5
...
replace ICCGroup = 0.027 if Analysis_bin == 4 

generate design_effect = 1 + (avg_cluster_size - 1) * ICCGroup if Analysis_bin == 4

replace EffectiveNGroup = PostNGroup / design_effect  if Analysis_bin == 4

replace PostHbA1cSEGroup = PostHbA1cSDGroup  / sqrt(EffectiveNGroup) if Analysis_bin == 4

replace PostHbA1cSDGroup = PostHbA1cSEGroup * sqrt(PostNGroup) if Analysis_bin == 4
...
replace PostHbA1cSDGroup = 2.22 if Analysis_bin == 6

replace design_effect = 1 + (avg_cluster_size - 1) * ICCGroup if Analysis_bin == 6

replace EffectiveNGroup = PostNGroup / design_effect  if Analysis_bin == 6

replace PostHbA1cSEGroup = PostHbA1cSDGroup  / sqrt(EffectiveNGroup) if Analysis_bin == 6

replace PostHbA1cSDGroup = PostHbA1cSEGroup * sqrt(PostNGroup) if Analysis_bin == 6
...
replace PostHbA1cSDGroup = 2.22 if Analysis_bin == 7

replace ICCGroup = 0.027 if Analysis_bin == 7 

replace design_effect = 1 + (avg_cluster_size - 1) * ICCGroup if Analysis_bin == 7

replace EffectiveNGroup = PostNGroup / design_effect  if Analysis_bin == 7

replace PostHbA1cSEGroup = PostHbA1cSDGroup  / sqrt(EffectiveNGroup) if Analysis_bin == 7

replace PostHbA1cSDGroup = PostHbA1cSEGroup * sqrt(PostNGroup) if Analysis_bin == 7


/*for studies with unadjusted SE, need to get unadjusted SD to recalculate adjusted SE using effective n*/
sort PostHbA1cSDGroup

replace PostHbA1cSDGroup = PostHbA1cSEGroup * sqrt(PostNGroup) if Analysis_bin == 1 & PostHbA1cSDGroup == .


bysort study_number  : egen n_arms = max(Arm)


save "analysis_Outcome_HbA1c_Model_1.dta", replace
...
save "analysis_Intervention_QI_Model_1.dta" , replace

use "analysis_Outcome_HbA1c_Model_1.dta" , clear

merge 1:1 RefID Arm using "analysis_Intervention_QI_Model_1.dta"
...
/*previously dropped in Lancet because no difference in arms, but if do 'proper MA', perhaps leave in?*/
/*
drop if RefID == 2915
drop if RefID == 5101
drop if RefID == 5943
...
keep study_number	Arm		Author_year		n_arms		RefID		Analysis_bin		avg_cluster_size		ICCGroup	///
	 NGroup 	PreHbA1cmeanGroup 	PreHbA1cSDGroup 	PreHbA1cSEGroup		/// 
	 EffectiveNGroup 	PostHbA1cmeanGroup   PostHbA1cSEGroup PostHbA1cSDGroup ///
...
outsheet inc_study_number  	avg_cluster_size  	ICCGroup* ///
		 ...
		 EffectiveNGroup* 	PostHbA1cmeanGroup* 	PostHbA1cSEGroup*  PostHbA1cSDGroup*   ///
							...
							PE* PSM* PR* ///
							CQI* FI* QI_other* ///
							n_arms using "cleaned_data_Model_1.txt" , replace comma
